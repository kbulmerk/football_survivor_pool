...
echo "[tuesday-open] Calling ${APP_URL}/api/cron/monday-results"
response=$(curl -s -w "\n%{http_code}" -X GET "${APP_URL}/api/cron/monday-results" \
  -H "Authorization: Bearer ${CRON_SECRET}")
http_code=$(echo "$response" | tail -n1)
body=$(echo "$response" | sed '$d')
echo "[tuesday-open] monday-results Response (${http_code}): ${body}"
[ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ] || { echo "[tuesday-open] monday-results FAILED with HTTP ${http_code}"; exit 1; }
